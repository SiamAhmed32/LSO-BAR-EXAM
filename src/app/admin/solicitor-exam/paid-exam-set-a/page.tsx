...
import { BookOpen, Edit, Trash2, Eye, Settings } from 'lucide-react';
...
const SolicitorPaidExam = () => {
	...
	const fetchQuestions = useCallback(async () => {
		try {
			setIsLoading(true);
			const response = await examApi.getQuestions('solicitor', 'set-a', currentPage, pageLimit);
	...
	const handleSubmitSettings = async (settings: any) => {
		try {
			await examApi.updateExamSettings('solicitor', 'set-a', settings);
	...
	const handleSubmitQuestion = async (question: Question) => {
		try {
			if (editingQuestion) {
				...
				await examApi.updateQuestion('solicitor', 'set-a', editingQuestion.id, question);
				toast.success('Question updated successfully');
			} else {
				// Create new question
				await examApi.createQuestion('solicitor', 'set-a', question);
	...
	const handleConfirmDelete = async () => {
		...
		try {
			setIsDeleting(true);
			await examApi.deleteQuestion('solicitor', 'set-a', questionToDelete.id);
		...
		<Box className='p-6'>
			<Box className='mb-8'>
				<h1 className='text-3xl font-bold text-primaryText mb-2 flex items-center gap-3'>
					...
					Solicitor Exam - Paid Set A
				</h1>
				<p className='text-gray-600 mb-4'>
					Manage Solicitor Paid Exam Set A questions
				</p>
