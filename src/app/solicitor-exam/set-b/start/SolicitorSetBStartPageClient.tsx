...
const SolicitorSetBStartPageClient = () => {
  ...
  useEffect(() => {
    ...
    const loadQuestions = async () => {
      try {
        ...
        const response = await examApi.getQuestions("solicitor", "set-b", 1, 200);
        ...
        if (!isMounted) return;
        console.error("Solicitor Set B - Failed to load questions:", err);
  ...
  return (
    <PaidExamAccessCheck examId="solicitor-set-b" examName="Solicitor Exam Set B">
      <Layout>
        <FreeExamRunner
...
export default SolicitorSetBStartPageClient;
